...
const Menu = ({ sidebarToggle, mobileView }) => {
  ...
  const [data, setMenuData] = useState(permissionList?.response);
...
<ul className="nk-menu">
  {data.length > 0 &&
    data.map((item, index) => {
      // Check if the item is active and not titled "FeedBack"
      if (item.status === "active" && item.title !== "FeedBack") {
        return (
          <MenuItem
            key={index}
            link={item.menuURL}
            ...
            text={item.title}
            sub={
              item.menuURL === "/dashboard" ||
              item.menuURL === "/home" ||
              item.menuURL === "/document/type/SOP" ||
              item.menuURL === "/document/type/DIY" ||
              item.menuURL === "/document/type/Other"||
              item.menuURL === "/document/type/Policy"
                ? ""
                : item.subMenu
            }
            sidebarToggle={sidebarToggle}
        ...
        // Return null for items with title "FeedBack" to exclude them
        return null;
      }
    })}
</ul>
