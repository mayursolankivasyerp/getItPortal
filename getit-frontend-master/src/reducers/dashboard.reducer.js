...
export const totalDashboardTicketAgents = createAsyncThunk(
  "/dashboard/agents",
  async ({ status, token }, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        `${ApiUrl}/dashboard/agents?status=${status}`,
...
const dashboardSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(totalDashboardTicketAgents.fulfilled, (state, action) => {
        if (action.payload) {
          return { ...state, loading: false, totalDashboardAgentList: action.payload };
        } else {
          return { ...state, loading: false, totalDashboardAgentList: initialState.totalDashboardAgentList };
        }
      })
      .addCase(totalDashboardTicketAgents.pending, (state) => {
        return { ...state, loading: true };
      })
